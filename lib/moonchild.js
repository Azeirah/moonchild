...
function initializeExtension(ext, deps, initFn, style) {
  var result;

  if (style) {
    // this is a really hacky way for handling plugin styles
    // it needs to be thought through again, especially because
    // embedding the entire plugin's stylesheet in a js string
    // is a terrible experience for plugin developers.
...
function registerExtension(id, deps, initFn, style) {
  ...
  ext.exports = initializeExtension(ext, deps, initFn, style);
